...
export const getCabecerasById = async (cab_id) => {
  const url = `https://proyecto-nosotros.herokuapp.com/ajustes/cab/:cab_id?pro_id=${cab_id}`;
  const response = await fetch(url);
  const data = await response.json();
  return data;
};
...
export const updateDetalle = async (det_id, det_cantidad, det_stock_registro) => {
  const url = `https://proyecto-nosotros.herokuapp.com/pajustes/det?det_id=${det_id}&det_cantidad=${det_cantidad}
